...
int tls_construct_server_hello(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    if (!s->hit &&
        ...
        servername = SSL_get_servername(SSL_CONNECTION_GET_SSL(s), TLSEXT_NAMETYPE_host_name);

        if (servername != NULL && (x509 = SSL_get_certificate(SSL_CONNECTION_GET_SSL(s))) != NULL
...
MSG_PROCESS_RETURN tls_process_client_compressed_certificate(SSL_CONNECTION *s,
                                                             PACKET *pkt)
{
