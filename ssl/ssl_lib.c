...
/* TODO: condition opt */
int BABASSL_get_master_key(SSL *ssl, unsigned char **master_key,
                           int *master_key_len)
{
    SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL(ssl);
...
int BABASSL_check_tlsext_status(SSL *ssl)
{
    SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL(ssl);
    ...
    if (sc->ext.status_type != -1 && SSL_CONNECTION_GET_CTX(sc) && SSL_CONNECTION_GET_CTX(sc)->ext.status_cb)
        return 1;
    else
        return 0;
}
...
void SSL_set_skip_scsv(SSL *ssl, int skip_scsv)
{
    SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL(ssl);
...
int SSL_set_cipher_list2(SSL *ssl, STACK_OF(SSL_CIPHER) *cipher_list)
{
    STACK_OF(SSL_CIPHER) *sk = NULL;
    SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL(ssl);
...
int SSL_set_cipher_list_by_id(SSL *ssl, STACK_OF(SSL_CIPHER) *cipher_list_by_id)
{
    STACK_OF(SSL_CIPHER) *sk = NULL;
    SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL(ssl);
...
STACK_OF(SSL_CIPHER) *SSL_dup_cipher_list(SSL *ssl)
{
    SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL(ssl);
...
STACK_OF(SSL_CIPHER) *SSL_dup_cipher_list_by_id(SSL *ssl)
{
    SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL(ssl);
...
int SSL_get_session_reused_type(SSL *ssl)
{
    SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL(ssl);
...
void SSL_set_status_callback(SSL *ssl,
...
void SSL_enable_sm_tls13_strict(SSL *ssl)
{
    SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL(ssl);
...
void SSL_disable_sm_tls13_strict(SSL *ssl)
{
    SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL(ssl);
